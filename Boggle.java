...
/*
 * currentWord JLabel not centered 
 * win messgae is weird
 * grid gets shrinked after it is clicked - also a problem with currentWord
 */
...
public class Boggle extends JFrame implements ActionListener {
	// game variables
	public static String[][] dices = { { "AAAFRS", "AEEGMU", "CEIILT", "DHHNOT", "FIPRSY" },
	...
	public int JButtonCoordinate[] = new int[2]; // Coordinate array of the pressed button
	static ArrayList<String> wordsFoundInGrid = new ArrayList<String>(); // all words found in grid, delete once pressed
	public static int minWordLength = 3; // defaulted
	public static String[] dictionary; // dictionary
	static int[] AIButtonXCoordinate;
	// stores the coordinates of each character of the word found by AI on the grid
	static int[] AIButtonYCoordinate;
	public int goalPoint = 15; // defaulted
	public static String[][] dicesFacingUp = new String[5][5]; // letters visible to user
	public static String[][] dicesFacingUpCopy = new String[dicesFacingUp.length][dicesFacingUp[0].length];
	// for recursion method
	public static int numButtonPressed = 0; // index of clickedButton
	public static int timeLimit = 15; // defaulted

	// checkers
	public static boolean Player1Turn = true;
	// check which player's turn, false if it's Player 2's turn
	public boolean gamePaused = false; // boolean for the pause/start button
	public static int[][] clickedButton = new int[dicesFacingUp.length * dicesFacingUp[0].length][2];
	// similar to AIButtonXCoordinate, but for user
	public static int sec = timeLimit; // decremented during countdown
	public static int totalPasses = 0; // total time the player has passed
	public static int numTimesAITimerTriggerred = 0;
	public static boolean AIWordShowedToUser = false;
	// creates a pause after AI word is shown on the grid
	public static String wordGuess = ""; // current word on GUI

	// user settings & scorekeeping
	public static String difficulty = "impossible";
	public static boolean isSingle = false;
	public static int Player1Score = 0;
	public static int Player2Score = 0;

	// utilities
	javax.swing.Timer timer = new Timer(1000, this); // triggers an event every second
	javax.swing.Timer AITimer = new Timer(250, this);
	// update a character of AIWord every quarter of a second

	// ###################### GUI Components ###########################
	// custom colours
	static Color darkTeal = new Color(8, 60, 95);
	static Color white = new Color(255, 253, 250);
	static Color lightBlue = new Color(157, 195, 239);
	...
	public void showMenu() { // construct menu
...
//		label.setPreferredSize(new Dimension(600, 50));
		...
		setting = new JPanel();
		GoalTF = new JLabel("Enter the goal point: ");
		GoalTF.setFont(baseFont);
		GoalTF.setPreferredSize(new Dimension(200, 20));
	...
	public void constructGame() { // construct boggle game
		...
		Goal = new JLabel("Goal: " + goalPoint);
		GoalTime.add(Goal);
		add(GoalTime);
	...
	public Boggle() {
		...
		showMenu();
	} // Ms. Andrighetti if you see this comment you should give us 100 :D, Chantal
		// wrote this
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(singlePlayer) || e.getSource().equals(multiPlayer)) {
			...
			try {
				...
				goalPoint = Integer.parseInt(GoalPrompt.getText());
	...
	public void checkWin() {
		if (Player1Score >= goalPoint) {
			showWinMessage(1);
		} else if (Player2Score >= goalPoint) {
			showWinMessage(2);
		}
	}
	...
	public void changePlayer() throws InterruptedException {
	...
	public static boolean elementOf(Object o, Object[][] array) {
	...
	public static int[] getJButtonCoordinate(Object b, Object[][] a) {
	...
	public static int getScore(String wordGuess) {
// check the score equivalent of the word
	...
	public static void restoreArray(int[][] a) {
		for (int r = 0; r < a.length; r++) {
			for (int c = 0; c < a[0].length; c++) {
				a[r][c] = -1;
			}
	...
	public static boolean buttonValid(int[] coor) {
	 ...
	 */
	static boolean findWord(String guess, int x, int y, int k) {
	...
	static boolean Start(String guess) {
	...
	public static void getWordsInGrid() {
		// sample board
	...
	public static void copy2DArray(String[][] original, String[][] copy) {
	 ...
	 */
	static boolean findCoor(String guess, int x, int y, int k) {
		// the wordGuess has matched and is correct
		if (k >= guess.length())
			return true;
		// findWord if the guess is out of bounds
	...
	static boolean startCoor(String guess) {
...
//play music
		AudioInputStream audio = AudioSystem.getAudioInputStream(new File("dreams.wav").getAbsoluteFile());// file is
																											// named
																											// dreams
	...
	static void buttonSound() throws UnsupportedAudioFileException, IOException, LineUnavailableException {
//makes a sound when the button is pressed
		AudioInputStream audio = AudioSystem.getAudioInputStream(new File("bClick.wav").getAbsoluteFile());// the file
																											// is called
																											// bCLick
		Clip clip = AudioSystem.getClip();
		clip.open(audio);
